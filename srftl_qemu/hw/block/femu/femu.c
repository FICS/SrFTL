...
void print_hmac(uint8_t hmac[32]) {
	  for (int i = 0; i < 32; i++) {
		      printf("%02x", hmac[i]);
			    }
	    printf("\n");
...
void enticing_update(struct ssd *ssd, char *buf, uint16_t buf_len) {
	    char enticing_flag[10];
	    int read_point = 0;
	    uint64_t start_lba;
	    uint64_t block_num;
	    memcpy(enticing_flag, buf, 10);
	    read_point += 10;

		if(strcmp(enticing_flag, "Enticing!") == 0) {
			printf("Enticing file get!\n");
			ssd->enticing = true;
			memcpy(&start_lba, buf + read_point, sizeof(uint64_t));
			read_point += sizeof(uint64_t);
			memcpy(&block_num, buf + read_point, sizeof(uint64_t));
			//      printf("lba %"PRIu64" len %"PRIu64"\n", start_lba, block_num);

			ssd->start_lba = start_lba;
			ssd->block_num = block_num;
		} else
			return;
}

void hmac_to_buffer(FemuCtrl *n, struct ssd *ssd, char *buf, uint16_t buf_len) {	
	struct suspicious_pg_info *submitted_tbl = ssd->immtbl_list->current_tbl;
	...
	bool is_print = 1;
	Error * err = NULL;
	if(ssd->immtbl_num > 1 && ssd -> no_processing ) {
		if (!qcrypto_hmac_supports(QCRYPTO_HASH_ALG_SHA256)) {
			printf("Not support hmac!\n");
            g_assert(0);
		}
	

                //printf("id: %d\n", ssd->immtbl_list->tbl_id);
	for(int i = 0; i < ssd->immtbl_list->tbl_sz; i++) {
		
		if(ssd->immtbl_list->tbl_type == TYPE_SUSTBL) {

                  printf("Impossible to handle sustbl\n");
                  exit(0);
                
			uint8_t *result = NULL;
			size_t resultlen = 0;

			memcpy(read_buf, (n->mbe).mem_backend + submitted_tbl[i].lba * 512, 4096);
			ret = qcrypto_hmac_bytes(ssd -> hmac, (const char *)read_buf, 4096, &result, &resultlen, &err);
			...
			if(is_print) {
//				memcpy(print_buf, read_buf, 9);
//				printf("Buffer value is: %s\n", print_buf);
				is_print = false;
			}
//			print_hmac(result);
			memcpy(buf + current_write_point, result, 32);
			current_write_point += 32;
			g_free(result);
		} else {
			if(submitted_tbl[i].rw_flag == 1) {
				uint8_t *result = NULL;
				size_t resultlen = 0;
...
//					printf("Buffer value is: %s\n", print_buf);
                                  ...
                                  //printf("lba: %"PRIu64" i: %d\n", submitted_tbl[i].lba * 512, i);
					is_print = false;
...
void immtbl_to_buffer(struct ssd *ssd, char *buf, uint16_t buf_len) {
//	printf("immtbl_num is %"PRIu16"\n", ssd->immtbl_num);
	
	if(ssd->immtbl_num > 1 && ssd -> no_processing ) {
		uint8_t test_buffer[4096];
		Error **errp;
		char *hash;
		char result_str[10] = "result!";
		uint16_t length = ssd->immtbl_list->tbl_sz;
		struct suspicious_pg_info *submitted_tbl = ssd->immtbl_list->current_tbl;
		int current_write_point = 0;
		uint8_t type = ssd->immtbl_list->tbl_type;
		uint16_t id = ssd->immtbl_list->tbl_id;

//		assert(ssd->immtbl_list != NULL);
		memset(buf, 0, buf_len);
		// buf = tbl_id + tbl_type + length + table
		// tbl = lba + rw_flag

		memcpy(buf, result_str, 10);
		current_write_point += 10;

		memcpy(buf + current_write_point, &id, sizeof(uint16_t));
		current_write_point += sizeof(uint16_t);

		memcpy(buf + current_write_point, &type, sizeof(uint8_t));
		current_write_point += sizeof(uint8_t);

		memcpy(buf + current_write_point, &length, sizeof(uint16_t));
		current_write_point += sizeof(uint16_t);
		
//		uint16_t print_type = type;
		//printf("Read, id is %"PRIu16", length is %"PRIu16"\n" , id, length);

		for(int i = 0; i < ssd->immtbl_list->tbl_sz; i++) {
			memcpy(buf + current_write_point, &(submitted_tbl[i].lba), sizeof(uint64_t));
			current_write_point += sizeof(uint64_t);
			
			memcpy(buf + current_write_point, &(submitted_tbl[i].rw_flag), sizeof(uint8_t));
			assert(submitted_tbl[i].rw_flag == 0 || submitted_tbl[i].rw_flag == 1);
			current_write_point += sizeof(uint8_t);

                        memcpy(buf + current_write_point, &(submitted_tbl[i].have_read), sizeof(uint8_t));
			current_write_point += sizeof(uint8_t);
		}
		uint8_t *result = NULL;
		size_t resultlen = 0;
...
//		printf("Table size (for HMAC) is: %d\n", current_write_point);

		ret = qcrypto_hmac_bytes(ssd -> hmac, (const char *)buf, current_write_point, &result, &resultlen, &err);
...
//		print_hmac(result);
		memcpy(buf + current_write_point, result, 32);
		g_free(result);
		assert(buf_len > current_write_point);
	 } else {
		char no_sus_str[10] = "Nothing!";
	 	memset(buf, 0, buf_len);
		
		memcpy(buf, no_sus_str, 10);
	 }
}

void get_result(struct ssd *ssd, char *buf, uint16_t buf_len) {
	uint16_t length;
	struct result *res = ssd->result;
	int current_read_point = 0;
	//Update the is_result and result in the ssd.
	uint16_t id;			   
	uint8_t mid_level;
	uint8_t flag;
	struct suspicious_pg_info *submitted_tbl = ssd->immtbl_list->current_tbl;
	uint16_t table_length = 0;
        ...
        if(memcmp(train, "start_train", 11) == 0) {
          printf("Start training!\n");
          ssd->start_train=true;
          ssd->stop_train=false;
          return;
        } 

        if(memcmp(train, "stop_train", 10) == 0) {
	...
	current_read_point += sizeof(uint16_t);

	memcpy(&length, buf + current_read_point, sizeof(uint16_t));
	current_read_point += sizeof(uint16_t);

	int len = length;
//	printf("length is %u id is %u\n", length, id);
//	printf("ssd->immtbl_list->tbl_id is %u\n", ssd->immtbl_list->tbl_id);

	if(ssd->is_result) {
		printf("Result not be resolved yet!\n");
		return;
	}

	if(ssd->immtbl_list->tbl_id != id) {
		printf("table number is %"PRIu16"\n", ssd->immtbl_num);
		printf("id number is not correct: %u\n", id);
		return;
	}

	

	if((length == ssd->immtbl_list->tbl_sz) && ssd->immtbl_list->tbl_id == id) {
	
//		printf("Catch result!\n");


		if(length != ssd->immtbl_list->tbl_sz) {
			printf("Length error!\n");
			return;
		}
		table_length = 2 * sizeof(uint16_t) + 2 * sizeof(uint8_t) * length;
		
		uint8_t *result = NULL;
		size_t resultlen = 0;
		Error * err = NULL;
		int ret;
		uint8_t table_hmac[32];

		ret = qcrypto_hmac_bytes(ssd -> hmac, (const char *)buf, table_length, &result, &resultlen, &err);
...
//		printf("Table length is : %d\n", table_length);
		...
		memcpy(table_hmac, buf + table_length, 32);
		if(memcmp(result, table_hmac, 32) != 0) {
			printf("HMAC not match!\n");
			print_hmac(result);
			print_hmac(table_hmac);
		}
//#endif
		g_free(result);
		...
		for(int i = 0; i < length; i++) {
			memcpy(&mid_level, buf + current_read_point, sizeof(uint8_t));
			if(mid_level <=3 && mid_level >=0)
				ssd->result[i].result_level = mid_level;
			else {
				printf("Result level error!\n");
				return;
			}
			current_read_point += sizeof(uint8_t);
	
			memcpy(&flag, buf + current_read_point, sizeof(uint8_t));
			current_read_point += sizeof(uint8_t);


			if(flag != submitted_tbl[i].rw_flag) {
				printf("Result flag error!\n");
				return;
			}

			ssd->result[i].rw_flag = submitted_tbl[i].rw_flag;

			ssd->result[i].lpn = submitted_tbl[i].lpn;
		}
			   
		ssd->is_result = true;
		ssd->no_processing = false;
	}
...
//		printf("NVMe lba is: %"PRIu64", length is: %"PRIu16"\n", req->slba, req->nlb);
	if(1.0 * n->ssd.high_num / n->ssd.marked_pg > 0.8 && n->ssd.high_num > 2000 && n->ssd.stop){
				n->ssd.stop = false;;
				printf("Ransomware happened!\n");
	}
	
	if(req->slba == TBL_READ_OFF && req->nlb == TBL_READ_SZ  && req->is_write == 0) {
		// 8192 < length < 16385
		//printf("NVMe lba is: %"PRIu64", length is: %"PRIu16"\n", req->slba, req->nlb);
	 	uint8_t p_buf[15000 + 1];

		immtbl_to_buffer(&(n->ssd), p_buf, 15000);
		
	    memcpy((n->mbe).mem_backend + TBL_READ_OFF * SECTOR_SZ, p_buf, 12288);

		ret = femu_rw_mem_backend_bb(&n->mbe, &req->qsg, data_offset, req->is_write);

		return NVME_TBL_PUSH;
    } else if (req->slba == HMAC_READ_OFF && req->nlb == HMAC_READ_SZ && req->is_write == 0) {
//		printf("HMAC!!!\n");
		
		hmac_to_buffer(n, &(n->ssd), n->ssd.buf, 32768);
	    memcpy((n->mbe).mem_backend + HMAC_READ_OFF * SECTOR_SZ, n->ssd.buf, 32768);
...
/////////////////////////////////////////////////
	else if(req->slba == WRITE_OFF  && req->nlb == WRITE_SZ && req->is_write == 1) {
//		printf("Catch result!\n");
		uint8_t p_buf[4001];

		ret = femu_rw_mem_backend_bb(&n->mbe, &req->qsg, data_offset, req->is_write);
		memcpy(p_buf, (n->mbe).mem_backend + WRITE_OFF * SECTOR_SZ, 4000);

		enticing_update(&(n->ssd), p_buf, 4000);
		get_result(&(n->ssd), p_buf, 4000);

		return NVME_RESULT_RETURN; 
	} else {
    	ret = femu_rw_mem_backend_bb(&n->mbe, &req->qsg, data_offset, req->is_write);
		if(req->is_write == 0) {
			struct suspicious_pg_info *submitted_tbl =n->ssd.immtbl_list->current_tbl;
			for(int i=0; i < n->ssd.immtbl_list->tbl_sz; i++ ){
				if(submitted_tbl[i].lba == slba)
					return NVME_TBL_PUSH;
		}
		}
	}
...
void nvme_process_sq_io(void *opaque)
{
    ...
    while (!(nvme_sq_empty(sq))) {
        ...
        if (1 || status == NVME_SUCCESS) {
			if(status == NVME_RESULT_RETURN || status == NVME_TBL_PUSH) {
				req->reqlat = 0;
				req->expire_time += 0;
				req->status = NVME_SUCCESS;
				femu_ring_enqueue(n->ssd.to_poller, (void *)&req, 1);
				//printf("After poller\n");
