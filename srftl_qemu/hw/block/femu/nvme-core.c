...
static uint8_t admin_key[16] = {0};

void flush_hotness(struct ssd *ssd) {
    uint64_t i = 0;
    struct ssdparams *spp = &ssd->sp;

    for(; i < spp->tt_pgs; i++) {
        fprintf(ssd->fp_hotness, "%"PRIu64",%"PRIu32",%"PRIu32"\n", i, ssd->freq_table[i].write_freq, ssd->freq_table[i].read_freq);
...
static void do_hmac() {
    QCryptoHashAlgorithm alg = QCRYPTO_HASH_ALG_SHA256;
    Error * err = NULL;
    int ret = 0;
    size_t resultlen = 0;
    ...
    QCryptoHmac * hmac = qcrypto_hmac_new(alg, (const uint8_t *)admin_key, sizeof(admin_key), &err);

    // see tests/test-crypto-hmac.c for more usage instructions
...
static uint16_t nvme_admin_cmd(FemuCtrl *n, NvmeCmd *cmd, NvmeCqe *cqe)
{
    ...
    case NVME_ADM_CMD_DELETE_CQ:
		...
		fclose(n->ssd.fp);
                ...
                fclose(n->ssd.fp_hotness);
                fclose(n->ssd.fp_valid);
		printf("****************************************************\n");
		printf("Total IO requests number is: %"PRIu64"\n", n->ssd.rw_total_number);
		printf("The read IO request number is: %"PRIu64", and the ratio of read is: %.6lf%\n", n->ssd.r_number, (100.0 * n->ssd.r_number)/n->ssd.rw_total_number);
                printf("The real data copies: %"PRIu64"\n", n->ssd.total_copies);
		printf("The write IO request number is: %"PRIu64", and the ratio of write is: %.6lf%\n", n->ssd.w_number, (100.0 * n->ssd.w_number)/n->ssd.rw_total_number);
		printf("The suspicious ransomware IO request (in-place write) number is: %"PRIu64", and the ratio of it is: %.6lf%\n", n->ssd.r_w_number, (100.0 * n->ssd.r_w_number)/n->ssd.rw_total_number);
                printf("The page number in SSD is: %"PRIu64"\n", n->ssd.sp.tt_pgs);
                printf("The current valid page number is: %"PRIu64", and the updated page number is: %"PRIu64"\n", n->ssd.written_page_num, n->ssd.update_page_num);
                printf("The updated write percentage to total pages is: %.6lf%\n", (100.0 * n->ssd.update_page_num) / n->ssd.written_page_num);
                printf("The read page number is: %"PRIu64"\n", n->ssd.read_page_num);
                printf("The read percentage to total pages is: %.6lf% \n\n", (100.0 * n->ssd.read_page_num) / n->ssd.written_page_num);



		printf("The suspicious ransomware IO request (range) number is: %"PRIu64", and the ratio of it is: %.6lf%\n", n->ssd.sus_range_num, (100.0 * n->ssd.sus_range_num)/n->ssd.rw_total_number);
		printf("The read number of range number is: %"PRIu64" \n", n->ssd.r_range);
		printf("The suspicious ransomware IO request  number is: %"PRIu64", and the ratio of it is: %.6lf%\n", n->ssd.identified_pg_num, (100.0 * n->ssd.identified_pg_num)/n->ssd.rw_total_number);
		printf("The benign IO request number is: %"PRIu64", and the ratio of it (in all the suspicious number) is: %.6lf%\n", n->ssd.fine_num, (100.0 * n->ssd.fine_num)/n->ssd.identified_pg_num);
		printf("The low probability IO request number is: %"PRIu64", and the ratio of it is (in all the suspicious number) is: %.6lf%\n", n->ssd.low_num, (100.0 * n->ssd.low_num)/n->ssd.identified_pg_num);
		printf("The middle probability IO request number is: %"PRIu64", and the ratio of it is (in all the suspicious number) is: %.6lf%\n", n->ssd.mid_num, (100.0 * n->ssd.mid_num)/n->ssd.identified_pg_num);
		printf("The high probability IO request number is: %"PRIu64", and the ratio of it is (in all the suspicious number) is: %.6lf%\n", n->ssd.high_num, (100.0 * n->ssd.high_num)/n->ssd.identified_pg_num);
