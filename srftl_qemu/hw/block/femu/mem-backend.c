...
/*	if (is_write) {
	    char p_buf[cur_len + 1];
	    memset(p_buf, 0, cur_len + 1);
	    memcpy(p_buf, mb + mb_oft, cur_len);
	    printf("Size is %"PRIu64", data offset is %"PRIu64" \n", cur_len, mb_oft);
	    printf("Buffer content is: \n %s \n", p_buf);	    
        }*/
/*	if (!is_write) {	
	    char p_buf[cur_len + 1];
            memset(p_buf, 0, cur_len + 1);
            memcpy(p_buf, mbe->mem_backend, cur_len);
